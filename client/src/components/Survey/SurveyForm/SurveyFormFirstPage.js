...
const SurveyFormFirstPage = props => {
    ...
    const limitDetails = () => {
        if (props.values?.specifyRecipients) {
            return (
                <div>
                    provide emails
                </div>
            )
        } else if (!props.values?.specifyRecipients) {
            return (
                <div>
                    <Field name="limit"
    ...
    let content;
    if (props.surveyCreatedSuccess) {
        content = <SurveyFormSuccess URL={props.surveyCreatedURL} password={props.surveyCreatedPass}/>;
    } else {
